...
def run_keras(X_train, y_train, X_val, y_val, X_test, y_test, layers, epochs, split=0, verbose=True):
    ...
    if X_val is None:
        ...
        print(X_train.shape)
        aa=model.fit(X_train, y_train, epochs=epochs, batch_size=batch, validation_data=(X_val, y_val),
                  callbacks=[history], verbose=verbose)
        acc=np.array(aa.history['val_accuracy'])
        loss=np.array(aa.history['val_loss'])
    ...
    if X_val is not None or split > 0:
        ...
        print ("\nLoss on validation set:"  + str(loss.mean()) + " Accuracy on validation set: " + str(acc.mean()))
    else:
        acc = None
    ...
    return model, history, acc.mean(), test_acc
  ...
  #               var.assign(initializer(var.shape, var.dtype))
    for trial in range(trials):
        ...
        #             initializer_method.run(session=session)
                    #v_arg.assign(initializer(v_arg.shape, v_arg.dtype))
        # Run the model
        ...
        if display:
            ...
            plt.plot(X_train, y, model)
            plt.title('Training data')
            plt.show()
            if X_test is not None:
                # plot classifier landscape on testing data
                plt.plot(X_test, y3, model)
...
run_keras_2d("3class", archs(2)[4], 10, split=0.5, display=False, verbose=False, trials=1 )
######################################################################
...
def run_keras_cnn_mnist(train, test, layers, epochs, split=0.1, verbose=True, trials=1):
    ...
    if test_acc:
        print ("\nAvg. test accuracy:"  + str(test_acc/trials))
# Example usage:
train, validation = get_MNIST_data(shift=20)
#train1=train
#validation1=validation
train1=(train[0]-np.amin(train[0]))/(np.amax(train[0])-np.amin(train[0]))
validation1=(validation[0]-np.amin(validation[0]))/(np.amax(validation[0])-np.amin(validation[0]))
train1=(train1,train[1])
validation1=(validation1,validation[1])
layers0=[Dense(input_dim=48*48, units=512, activation="relu"),
         Dense(units=256, activation="relu"),
         Dense(units=10, activation="softmax")]
layers = [Conv2D(32,(3,3),input_shape=(48,48,1), activation='relu'),
...
#run_keras_fc_mnist(train1, validation1, layers0, 1, split=0.1, verbose=True, trials=5)

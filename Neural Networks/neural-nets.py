...
class Tanh(Module):  # Layer activation
    ...
    def backward(self, dLdA):  # Uses stored self.A
        ...
        dAdZ=np.where(self.Z !=0,4.0/(2.71828**self.Z + 2.71828**-self.Z)**2,1.0)
        for i in range(b):
            z1=dAdZ[:,i:i+1]
            #print(z1)
            d=a*z1  #calculates dAdz
            #print(d)
            dldz=np.dot(d,dLdA[:,i:i+1]) #dAdz.dLdA
            #print(dLdA[:,i:i+1])
            if i==0:
                dLdZ=dldz
            else:
                dLdZ=np.concatenate((dLdZ,dldz),axis=1)
        #print(self.A)
        return dLdZ  # Your code: return dLdZ (n, b)


class ReLU(Module):  # Layer activation
    def forward(self, Z):
        self.A = np.where(Z>0,Z,0)  # Your code: (n, b)
        return self.A
...
class SoftMax(Module):  # Output activation
    def forward(self, Z):
        self.Z=Z
        ze=2.71828**Z
        zsum=np.sum(ze,axis=0)
        return ze/zsum  # Your code: (?, b)
    ...
    def class_fun(self, Ypred):  # Return class indices
        print(Ypred)
        return np.argmax(Ypred,axis=0)  # Your 
...
class NLL(Module):  # Loss
    def forward(self, Ypred, Y):
        self.Ypred = Ypred
        self.Y = Y
        pred_log=np.log(self.Ypred)
        arr=np.sum(self.Y*pred_log)
        return -np.sum(arr)  # Your code: return loss (scalar)

    def backward(self):  # Use stored self.Ypred, self.Y
        return -self.Y+self.Ypred  # Your code for dLdZ (?, b)
...
class Sequential:
    ...
    def sgd(self, X, Y, iters=100, lrate=0.005):  # Train
        ...
        for it in range(iters):
            ...
            loss1=self.loss
            loss2=loss1.forward(ypred,y)
            i=len(self.modules)
            for m in self.modules[::-1]:
                if i==len(self.modules):
                    delta1=loss1.backward()
                    #print(delta1)
                    #print(i)
                else:
                    delta1=m.backward(delta1)
                    m.sgd_step(lrate)
                    #print(delta1)
                    #print(i)
                i=i-1

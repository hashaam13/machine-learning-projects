...
def sgd_step(data, x, lam, step):
    ...
    (u, b_u, v, b_v) = x
    (lam_u, lam_v) = lam
    ua=u[a]-step*((np.dot(u[a].T,v[i])+b_u[a]+b_v[i]-r)*v[i]+lam_u[a]*u[a])
    vi=v[i]-step*((np.dot(u[a].T,v[i])+b_u[a]+b_v[i]-r)*u[a]+lam_v[i]*v[i])
    b_ua=b_u[a]-step*(np.dot(u[a].T,v[i])[0]+b_u[a]+b_v[i]-r)
    b_vi=b_v[i]-step*(np.dot(u[a].T,v[i])[0]+b_u[a]+b_v[i]-r)
    u[a]=ua
    v[i]=vi
    b_u[a]=b_ua
    b_v[i]=b_vi
...
fav=0
# for m in movies:
#     if 'Children' in genres_dict[m]: 
#         fav+=1
#print(fav)
#model = mf_als(data, None, k=10, lam=1, max_iter=20)
#save_model(model)
(u,bu,v,bv)=load_model()
#u=u[270894]
# yes=np.dot(u.T,v)
# yes=yes[0,:][:,0]
# rat=yes+bv+bu[270894]
...
# sims = []
# for key in keys:
#     if 'Comedy' in genres_dict[key]:
#         dic0[key]=movies_dict[key]
# for g in genres:
#     if g=='Comedy': continue
#     print(g)
#     for key in keys:
#         if g in genres_dict[key]:
#             dic[key]=movies_dict[key]
#     for i in dic0:
#         for j in dic:
#             if i == j: continue
...
for k in keys:
    vr=v[k]
    for key in keys:
        dic[key]=np.dot(vr.T,v[key])[0,0]/(np.linalg.norm(v[key])*np.linalg.norm(vr))
    nn=np.array(list(dic.values()))
    ll=ll+(nn.sum()-1)/1999
print(ll)
sorted_d = dict( sorted(dic.items(), key=operator.itemgetter(1),reverse=True))
ii=0
for kk in sorted_d.keys():
    print(movies_dict[kk])
    ii+=1
    if ii>16:
        break
